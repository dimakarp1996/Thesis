...
\begin{itemize}
\item Метки из всех наборов данных, кроме SST-2, считаются на 50\% положительных и на 50\% отрицательными.
...
\hline
Название эксперимента & Среднее& \({RTE}\)& \({QQP}\)&\({MNLI}\)&\({SST-2}\)\\
\hline
\hline
...
\hline
Название эксперимента &Среднее& \({RTE}\)& \({QQP}\)&\({MNLI-m}\)&\({MNLI-mm}\)&\({SST-2}\)\\
\hline
\hline
